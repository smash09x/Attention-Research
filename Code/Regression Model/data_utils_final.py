...
# DVs: log_decay_speed = full peak width, log_decay_speed_post = post-peak width only
DECAY_OUTCOMES = ('log_decay_speed', 'log_decay_speed_post')


def load_sheets(trends_path, strat_path, youtube_path=None):
    # YouTube CSV is optional (only needed for models B/C)
...
def build_main(trends, strat, youtube=None, extra_vars=()):
    ...
    # Drop rows that never decayed within the measured window
    ...
    # post_peak_width_days may not exist in every trends file (e.g. age-proxy set);
    # compute it only where present, and don't let it drop rows from the main outcome
    ...
    if extra_vars:
        ...
        # Inner merge: keep only keyword/era rows that have YouTube data
