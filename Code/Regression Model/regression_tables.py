...
Builds Table 1, Table A5, Table A6 and Table A7 of the paper.

  Table 1  Platform-driven effect per era, as a duration ratio
  Table A5 Model A refitted with era as an unordered factor, plus the nested F test
  Table A6 Leave-one-category-out refits and median peak widths
  Table A7 Rows excluded by censoring, by era and platform-driven status

Tables A1 to A4 come from model_main.py. Table A8 comes from survival_models.py.

    python regression_tables.py all
    python regression_tables.py 1
"""
...
def table_1(df):
    """
    Platform-driven effect evaluated separately in each era, as a linear
    combination of the main effect and the interaction, converted to exp(-b).
    """
    ...
    for era in (0, 1, 2):
        ...
        "Ratio is exp(-b), so the confidence bounds swap when negated."
...
def table_a5(df):
    """Model A refitted with era as an unordered factor, both DVs, plus the nested F test."""
    print("\nTable A5")
    ...
    "The nested F test uses the classical covariance, which is what an F test assumes."
...
def table_a6(df):
    """Leave-one-category-out refits of the era x platform interaction, and median peak widths."""
    df = add_category(df)
    print("\nTable A6")
...
if __name__ == '__main__':
    parser = add_common_args(argparse.ArgumentParser(
        description="Build Table 1, A5, A6 and A7."))
    parser.add_argument('table', choices=['1', 'A5', 'A6', 'A7', 'all'])
