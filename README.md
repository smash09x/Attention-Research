...
cd "Code/Regression Models"
python run_all.py
```

`run_all.py` runs every analysis script in order on the CSVs in `Data/CSVs/` and
saves each script's printed output, plus `figure1.png`, to `Code/Regression Models/results/`.
...
Code/
  ...
  Regression Models/
    ...
    regression_tables.py   Table 1, A5, A6, A6b, A7
...
| Table 1, A5, A6, A6b, A7 | `python regression_tables.py all` (or one of `1`, `A5`, `A6`, `A6b`, `A7`) |
...
| Age-proxy Cox, platform only | HR = 3.343, 95% CI [1.360, 8.221], p = .009 |
| Age-proxy AFT era x platform, keyword-clustered p | .956 Weibull, .803 lognormal, .875 log-logistic |
| LR test, all era terms (Weibull AFT) | chi2(3) = 2.51, p = .473 |
| AFT log-time AIC, no era terms | 671.2 lognormal, 679.2 log-logistic, 693.5 Weibull |
| Shorts share, four H3 specifications | p = .899, .846, .846, .418 |
| Viral era slope (Table A6b) | b = 1.070, 95% CI [0.475, 1.666], p < .001 |
